...
To better assess the variation of PAPER-128 data across its two-year deployment (which consists of 7+ individual epochs of data, where an epoch is characterized by the shut-down and re-starting of the correlator), we first investigate one-dimensional slices of compressed data that span the entire time axis but only one frequency channel. We choose channel 100 for this analysis, as its corresponding frequency of 1.5\,GHz lies in the middle of our band (similar to PAPER-64, PAPER-128's bandwidth consists of 203 frequency channels ranging from 100 to 200\,MHz).
...
\begin{figure*}
	...
	\caption{Visibility amplitudes as a function of LST for different Julian days of data. The left column shows the data before (top) and after (bottom) the flagging of outlier days for Epoch 1. The right column shows similar data for Epoch 2.}
...
V_{128} = be^{i\phi_{b}},
\end{eqnarray}
then we solve for an overall gain correction factor as:
...
\end{equation}
Combining these two gives the bandpass solution (one number per frequency):
...
% interpret PS plots, what is the best limit, and how does it compare to PSA64?
